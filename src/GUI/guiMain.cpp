...
// Test d'interface graphique : les manipulations des tuiles peuvent se faire au
// clavier
void guiMain::play(SolarSystem *system) {
  ...
  while (window.isOpen() && sliderWindow.isOpen()) {
    ...
    const sf::Time freezeLength{
        sf::milliseconds(1000.0 / GlobalValues::PHYSICS_TICK_SPEED)};
...
void guiMain::drawPaths(SolarSystem *system, RenderWindow *window) {
  for (const auto &kv : system->getPaths()) {
    for (unsigned int i = 1; i < kv.second.size(); i++) {
      ...
      line[1].position =
          sf::Vector2f(kv.second[i - 1].getX(), kv.second[i - 1].getY());
...
void guiMain::zoomViewAt(sf::Vector2i pixel, sf::RenderWindow &window,
                         float zoom) {
...
SliderSFML guiMain::getSliderTimeScale() { return sliderTimeScale; }

SliderSFML guiMain::getSliderPathSize() { return sliderPathSize; }