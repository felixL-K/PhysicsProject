...
CelestialBody::CelestialBody(SolarSystem *system)
    : diameter{10}, mass{10}, position{Vector2D{0, 0}},
      velocity{Vector2D{0, 0}},
      color{(sf::Uint8)(rand() % 255), (sf::Uint8)(rand() % 255),
            (sf::Uint8)(rand() % 255)},
      solarSystem{system} {}

CelestialBody::CelestialBody(double massIn, Vector2D posIn, SolarSystem *system)
    : diameter{10}, mass{massIn}, position{posIn}, velocity{Vector2D{0, 0}},
      color{(sf::Uint8)(rand() % 255), (sf::Uint8)(rand() % 255),
            (sf::Uint8)(rand() % 255)},
      solarSystem{system} {}

CelestialBody::CelestialBody(double diameter, double massIn, Vector2D posIn,
                             Vector2D velocity, SolarSystem *system)
...
CelestialBody::CelestialBody(const CelestialBody &vect)
    : diameter{vect.getDiameter()}, mass{vect.getMass()},
      position{vect.getPosition()}, velocity{vect.getVelocity()},
      color{(sf::Uint8)(rand() % 255), (sf::Uint8)(rand() % 255),
            (sf::Uint8)(rand() % 255)} {}
...
// getters & setters
double CelestialBody::getDiameter() const { return diameter; }
...
sf::Color CelestialBody::getColor() const { return color; }
void CelestialBody::setDiameter(double x) { diameter = x; }
void CelestialBody::setMass(double x) { mass = x; }
void CelestialBody::setPosition(Vector2D x) { position = x; }
void CelestialBody::setVelocity(Vector2D x) { velocity = x; }
...
void CelestialBody::addVelocity(Vector2D forceIn) {
  velocity.add(forceIn.getX() * GlobalValues::timeScale,
               forceIn.getY() * GlobalValues::timeScale);
}
...
void CelestialBody::newtonGrav(CelestialBody *p) {
  ...
  if (dist != 0) {
    double force = GlobalValues::GRAVITATIONALCONSTANT * this->mass * p->mass /
                   pow(dist, 2);
...
void CelestialBody::drawObject(sf::RenderWindow *window) {
  ...
  circle1.setPosition(position.getX() - diameter / 2,
                      position.getY() - diameter / 2);
