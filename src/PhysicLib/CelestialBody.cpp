...
CelestialBody::CelestialBody(double diameter,
                             double massIn,
                             Vector2D posIn,
                             Vector2D velocity,
                             SolarSystem *system)
...
CelestialBody::CelestialBody(const CelestialBody &vect)
    : diameter{vect.getDiameter()}
    , mass{vect.getMass()}
    , position{vect.getPosition()}
    , velocity{vect.getVelocity()}
    , color{rand() % 255, rand() % 255, rand() % 255} {}
...
void CelestialBody::setColor(int r, int g, int b) { color = sf::Color(r, g, b); }
...
double CelestialBody::distance(CelestialBody *p) { return position.distance(p->getPosition()); }
...
void CelestialBody::drawObject(sf::RenderWindow *window) {
    ...
    circle1.setPosition(
        sf::Vector2f(position.getX() - diameter / 2, position.getY() - diameter / 2));
